...
void requestEvent() {
    // I2C 통신 사이에 200ms 간격을 두어 안정성을 높임
    static unsigned long lastRequestTime = 0;  // 마지막 요청 시간 저장 변수
    unsigned long currentMillis = millis();    // 현재 시간 가져오기

    if (currentMillis - lastRequestTime > 10) { // 200ms 간격으로 I2C 데이터 요청
        lastRequestTime = currentMillis;  // 마지막 요청 시간 갱신
...
// ISR: 팬의 tach 센서를 통해 RPM 카운트를 증가시킴
